...
<script setup>
    ...
    (newBatchNo, oldBatchNo) => {
        ...
        let find=validateItem(props.index);
        if (!find){
            props.selected_serial_no=[]
    ...
    // Computed property for quantity
    const quantity = computed({
    get() {
        return props.item.has_serial_no ? props.item.selected_serial_no.length : props.item.qty;
    },
    set(value) {
        if (props.item.has_serial_no) {
        props.item.selected_serial_no.length = value;
        } else {
    ...
    // Watch for changes in selected_serial_no and update item.qty
    watch(
    () => props.item.selected_serial_no.length,
    (newLength) => {
        if (props.item.has_serial_no) {
        props.item.qty = newLength;
        }
    }
    );
